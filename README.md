...
Latency: splitting the flow means each request is **one** Gemini render (build) or a single fast call (brief), so the build route sets `maxDuration = 120` and the brief route is well under the Hobby 60 s cap — no Vercel Pro required. Brief and build think at `thinkingLevel: HIGH`, the critic at `MEDIUM`. The client logic lives under [`app/generate/flow/`](app/generate/flow) — the [`useGenerateFlow`](app/generate/flow/useGenerateFlow.ts) hook (state + handlers), the documented [`phase`](app/generate/flow/phase.ts) machine, and the [`requests`](app/generate/flow/requests.ts) layer; [`GenerateForm.tsx`](app/generate/GenerateForm.tsx) is a thin orchestrator that routes each phase to a markup-only view in [`app/generate/ui/`](app/generate/ui).
...
The agent pipeline lives in [`lib/agent/`](lib/agent): the prompts under [`stages/`](lib/agent/stages), split by role — [`brief.ts`](lib/agent/stages/brief.ts) (creative direction → structured JSON), [`build.ts`](lib/agent/stages/build.ts) (the mechanical build + revise), [`qa.ts`](lib/agent/stages/qa.ts) (the critic) — sharing common fragments (the input block, cover handling, the no-fabrication contract) and request parsing from [`input.ts`](lib/agent/input.ts), with the per-stage Gemini calls wrapped in [`run-brief.ts`](lib/agent/run-brief.ts) / [`run-build.ts`](lib/agent/run-build.ts) so the route handlers stay thin. What the prompts enforce:
...
The prompt history is in git: `git log --follow lib/agent/input.ts` covers the single mega-prompt era (vague → role+format → context/discipline/anti-clichés → `:scope`/`@import` → banning hallucinated URLs → the contrast / scroll-timeline / responsive-cover rules), before it was split into the `brief.ts` / `build.ts` / `qa.ts` pipeline above.
...
- `app/api/generate/brief/route.ts`, `build/route.ts` — the two human-gated stages, thin handlers over the pipeline (brief JSON; build NDJSON streaming over a ReadableStream)
- `lib/agent/` — the agent pipeline: prompts in `stages/{brief,build,qa}.ts`, shared fragments + request parsing in `input.ts`, the build/QA orchestration in `run-build.ts` (and `run-brief.ts`), cover handling in `cover.ts`
- `lib/generate/protocol.ts` — the streaming wire contract shared by the build route and the client
- `lib/images/dimensions.ts` — dependency-free cover intrinsic-size decode (tests alongside)
- `app/generate/flow/` + `GenerateForm.tsx` + `ui/` — the flow hook & `phase` machine (stream consumer, abort-on-resubmit) in `flow/`, a thin orchestrator, and the markup-only view components in `ui/`
